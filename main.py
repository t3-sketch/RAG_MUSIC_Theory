...
async def rag_ingest(ctx: inngest.Context) -> dict:
    ...
    source_id = ingest.url_to_source_id(data.url)

    # ── step1: スクレイプ ───────────────────────────
    # ingest.scrape は list[dict] を返す。
    # main.py（接着剤）で ScrapeEntry に詰める。
    entries: list[dict] = await ctx.step.run(
        "scrape",
        lambda: ingest.scrape(data.url),
    )

    # ── step2: チャンク分割 ─────────────────────────
    # ingest.chunk は list[dict] を返す（{"text","source","chunk_index"}）。
    chunks: list[dict] = await ctx.step.run(
        "chunk",
        lambda: ingest.chunk(entries, source_id),
    )
